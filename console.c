...
draw_messages(f32 dt) {
    ...
    for (int i = 0 ; i < array_count(messages); i++) {
        ...
        switch(message->kind) {
            case MESSAGE_INT: {
                draw_number(message->int_val, p, 2.5f, 0xffffff, 1);
            } break;
            ...
            case MESSAGE_V2I: {
                draw_number(message->v2i_val.x, p, 2.5f, 0xffffff, 1);
                draw_number(message->v2i_val.y, add_v2(p, (v2){10, 0}), 2.5f, 0xffffff, 1);
            } break;
