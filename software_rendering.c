...
global_varible v2 cam_p, cam_dp;
global_varible f32 cam_scale = 0.01f;
...
internal f32
draw_number(int number, v2 p, f32 size, u32 color, int number_of_leading_zeros) {
...
draw_f32(f32 num, v2 p, f32 size, b32 color) {
    ...
    p.x = draw_number(frac, p, size, color, 2);
    draw_rect(add_v2(p, (v2){.5f, -1.f}), (v2){.25f, .25f}, color);
    p.x -= 1.f;
    draw_number(whole, p, size, color, 1);
}
...
draw_transparent_rotated_rect(v2 p, v2 half_size, f32 angle, u32 color, f32 alpha) { //In degrees
    ...
    m2 world_to_pixels_cam_scale_transform = (m2){
        s, 0,
        0, s,
    };
    ...
    for (int i = 0; i < 4; i++) {
        ...
        rect.p[i] = mul_m2_v2(world_to_pixels_cam_scale_transform, rect.p[i]);
...
struct {
    u32 *pixels;	//was previous u32 *pixels
	void *memory;
    int width, height, pitch;
	f32 AlignPercentageX; //Not sure of a better way to default these to 0.5
	f32 AlignPercentageY; //Not sure of a better way to default these to 0.5
} typedef Bitmap;
...
make_empty_bitmap(int width, int height){
	...
	result.pitch = SafeTruncateToUInt16(result.width*4);
	int TotalBitmapSize = width*height*4; //TODO: Replace 4 with defined variable called BITMAP_BYTES_PER_PIXEL
	result.memory = malloc(TotalBitmapSize);
	result.pixels = result.memory;
	//TODO: This is where we would push the size of the bitmap onto the memory arena, if we had a memory arena
...
load_png(char *path) {
    ...
    result.AlignPercentageX = 0.5f;
	result.AlignPercentageY = 0.5f;
...
draw_bitmap(Bitmap *bitmap, v2 p, v2 half_size, f32 alpha_multiplier) {
	...
	v2 Align = (v2){(0.5f-bitmap->AlignPercentageX) * (half_size.x * 2.0f),  (0.5f - bitmap->AlignPercentageY) * (half_size.y*2.0f)};
	p.x = p.x + Align.x;
	p.y = p.y + Align.y;
    p = sub_v2(p, cam_p);
    //draw_rect(p, half_size, 0xff0000);
    ...
    for (int y = y0; y < y1; y++) {
        ...
        int stride = (int)(v*(f32)bitmap->height)*bitmap->width;
        u32 *source_pixels = (u32 *)bitmap->pixels + stride;
        for (int x = x0; x < x1; x++) {
...
get_letter_index(char c) {
	...
	if(c >= 97)
	{
		c = c-'a'+26; //bitmaps are indexed in fontbitmaps. Bitmaps 27 onwards are lower case letters. Note this dumb. Please dont 
	} else
	{
		c = c-'A';
	}
...
get_word_align_offset(int text_align, char *word, f32 size) {
    ...
    for (char *at = word; *at && *at != '\\'; at++) {
		...
		Bitmap font_bitmap = font_bitmaps[letter_index].bitmap;
        f32 width_modifier = (font_bitmap.width/256.0f)* 1.0f;
		result += (size*1.0f)*width_modifier;
    }
...
internal void
draw_text(char *text, v2 p, f32 size, int text_align)
{
	...
	b32 first_line = true;
	 ...
	 for (char *letter_at = text; *letter_at != '\0'; letter_at++) {
		...
		else 
		{
			...
			{
				p.x = first_x;
				p.x += get_word_align_offset(text_align, letter_at, size);
				original_x = p.x;
				first_line = false;
			}
			u32 CodePoint = *letter_at;
			r32 AdvanceX = 0.0f;
			if(CodePoint != ' ')
			{
				f32 height_modifier = (font_bitmap.height / 256.0f) * 1.0f;
				f32 width_modifier = (font_bitmap.width/256.0f)* 1.0f;
				if (*letter_at != ' ') 
				{
					draw_bitmap(&font_bitmap, p, (v2){(size*0.5f)*width_modifier, (size*0.5f)*height_modifier}, 1.0f); 
				}
				AdvanceX = (size*1.1f)*width_modifier;
				p.x += AdvanceX;
...
internal void
draw_block_text(char *text, v2 p, f32 size, u32 *colors, int color_count, int first_color, int text_align) {
