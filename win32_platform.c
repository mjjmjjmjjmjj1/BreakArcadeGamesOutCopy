...
global_varible f32 current_time;
...
os_read_entire_file(char *file_path) {
    ...
    result.data = VirtualAlloc(0, (SIZE_T)result.size,
...
global_varible LPDIRECTSOUNDBUFFER win32_sound_buffer;
global_varible Game_Sound_Buffer sound_buffer;
    ...
    if(DSoundLibrary)
    {
        ...
        if(DirectSoundCreate && SUCCEEDED(DirectSoundCreate(0, &DirectSound, 0)))
        {
			if(SUCCEEDED(DirectSound->lpVtbl->SetCooperativeLevel(DirectSound, window, DSSCL_PRIORITY))) 
			{
					...
					DirectSound->lpVtbl->CreateSoundBuffer(DirectSound, &buffer_description, &primary_buffer, 0);
				...
				if (SUCCEEDED(DirectSound->lpVtbl->CreateSoundBuffer(DirectSound, &buffer_description, &win32_sound_buffer, 0))) {
					// Success
				} else {
					invalid_code_path;
				}
...
win32_toggle_fullscreen(HWND window) {
    ...
    if (style & WS_OVERLAPPEDWINDOW) {
        ...
        if (GetWindowPlacement(window, &win32_window_position) &&
        ...
        SetWindowPlacement(window, &win32_window_position);
    ...
    switch(message) {
        ...
        case WM_ACTIVATEAPP: {
            if (w_param){
				...
				SetCursor(win32_cursor); // Test more thorougly
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
    ...
    HWND window = CreateWindowExA(0, window_class.lpszClassName, "Breakout", WINDOW_FLAGS, 0, 0, 1280, 720, 0, 0, 0, 0);
	...
	f32 target_dt = 0.01666f; //Get user refresh rate?
    ...
    // Audio stuff
    {
        ...
        os_add_job_to_queue(&audio_queue, win32_update_audio, 0);
    }
...
#define process_button(vk, b) \
        ...
        // Mouse Input
        {
            ...
            win32_input.mouse_dp = 
				sub_v2i(new_mouse_p, win32_old_mouse_p);
			
            if (win32_lock_mouse){
				SetCursorPos(win32_center_screen.x, win32_center_screen.y);
				win32_old_mouse_p = win32_center_screen;
			} else {
		...
		int sleep = (int)((target_dt-dt_before_sleep)*500.0f);//*1000.f);
		if(sleep > 1)	{
